...
impl SchemaValidator {
    ...
    pub fn validate_shared_args(schema: &InputSchema, args: &SharedArgs) -> Result<(), DCPError> {
        let mut present_mask = 0u64;
        let mut declared_data_len = 0usize;
        ...
        for (idx, field) in schema.fields.iter().enumerate() {
            ...
            let actual_type = arg_type_from_layout(args.layout(), idx)?;
            if actual_type == ArgType::Null {
                continue;
            }
            ...
            let end = offset.checked_add(size).ok_or(DCPError::OutOfBounds)?;
            if end > args.data().len() {
                return Err(DCPError::OutOfBounds);
            }
            declared_data_len = declared_data_len.max(end);

            if field.enum_range.is_some() {
                let value = *args.data().get(offset).ok_or(DCPError::OutOfBounds)?;
                Self::validate_enum(field, value)?;
            }
        ...
        for idx in schema.fields.len()..16 {
            if arg_type_from_layout(args.layout(), idx)? != ArgType::Null {
                return Err(DCPError::ValidationFailed);
            }
        }

        if args.data().len() > declared_data_len {
            return Err(DCPError::ValidationFailed);
        }
...
fn arg_type_from_layout(layout: u64, index: usize) -> Result<ArgType, DCPError> {
    let shift = index.checked_mul(4).ok_or(DCPError::ValidationFailed)?;
    let type_bits = ((layout >> shift) & 0xF) as u8;
    ArgType::from_u8(type_bits).ok_or(DCPError::ValidationFailed)
}
...
mod tests {
    ...
    fn test_enum_validation() {
        ...
        assert_eq!(
            SchemaValidator::validate_enum(&field, 0),
            Err(DCPError::ValidationFailed)
        );
        assert_eq!(
            SchemaValidator::validate_enum(&field, 6),
            Err(DCPError::ValidationFailed)
        );
    }
