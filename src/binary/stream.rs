...
const ALL_CHUNK_FLAGS: u8 =
    ChunkFlags::FIRST | ChunkFlags::CONTINUE | ChunkFlags::LAST | ChunkFlags::ERROR;

fn valid_chunk_flags(flags: u8) -> bool {
    if flags == 0 || flags & !ALL_CHUNK_FLAGS != 0 {
        return false;
    }
    if flags & ChunkFlags::ERROR != 0 {
        return flags == ChunkFlags::ERROR;
    }
    matches!(
        flags,
        ChunkFlags::FIRST | ChunkFlags::CONTINUE | ChunkFlags::LAST
    ) || flags == (ChunkFlags::FIRST | ChunkFlags::LAST)
}
