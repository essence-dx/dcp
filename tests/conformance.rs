...
impl ConformanceTestSuite {
    ...
    fn test_tools_list(&mut self) {
        ...
        let valid = tools
            .map(|t| t.iter().all(|tool| tool.get("name").is_some()))
            .unwrap_or(false);
    ...
    fn test_resources_list(&mut self) {
        ...
        let valid = resources
            .map(|r| {
                r.iter()
                    .all(|res| res.get("uri").is_some() && res.get("name").is_some())
            })
            .unwrap_or(false);

        self.add_result(
            "resources_list_format",
            "Resources",
            valid,
            None,
            vec!["11.1"],
    ...
    fn test_prompts_get(&mut self) {
        ...
        let valid = messages
            .map(|m| {
                m.iter()
                    .all(|msg| msg.get("role").is_some() && msg.get("content").is_some())
            })
            .unwrap_or(false);
    ...
    fn test_request_id_types(&mut self) {
        ...
        let all_valid = valid_ids
            .iter()
            .all(|id| id.is_number() || id.is_string() || id.is_null());
