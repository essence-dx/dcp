...
use dcp::binary::{ArgType, BinaryMessageEnvelope, Flags, MessageType};
...
mod tests {
    ...
    #[test]
    fn schema_rejects_required_field_missing_from_shared_args_layout() {
        let mut schema = InputSchema::new();
        schema.add_field(FieldDef::new("enabled", ArgType::Bool, 0, 1));
        schema.set_required(0);

        let args = SharedArgs::new(&[1], 0);

        assert_eq!(
            SchemaValidator::validate_shared_args(&schema, &args),
            Err(DCPError::ValidationFailed)
        );
    }

    #[test]
    fn schema_rejects_shared_args_type_mismatch() {
        let mut schema = InputSchema::new();
        schema.add_field(FieldDef::new("enabled", ArgType::Bool, 0, 1));

        let args = SharedArgs::new(&[1], ArgType::String as u64);
    ...
    #[test]
    fn schema_rejects_shared_args_out_of_bounds_field() {
        let mut schema = InputSchema::new();
        schema.add_field(FieldDef::new("count", ArgType::I32, 0, 4));

        let args = SharedArgs::new(&[1, 2, 3], ArgType::I32 as u64);
    ...
    #[test]
    fn parser_rejects_trailing_bytes_after_declared_payload() {
        let envelope = BinaryMessageEnvelope::new(MessageType::Tool, 0, 1);
        let mut message = envelope.as_bytes().to_vec();
        message.extend([0xAA, 0xBB]);
    ...
    fn parser_rejects_reserved_flag_bits() {
        let envelope = BinaryMessageEnvelope::new(MessageType::Tool, 0b1000_0000, 0);
        let message = envelope.as_bytes().to_vec();
    ...
    #[test]
    fn parser_rejects_unknown_message_type_before_dispatch() {
        let envelope = BinaryMessageEnvelope::new(MessageType::Tool, 0, 0);
        let mut message = envelope.as_bytes().to_vec();
        message[2] = 0;
