...
fn arb_json_message() -> impl Strategy<Value = String> {
    ...
    .prop_map(|chars| {
        ...
        format!(
            r#"{{"jsonrpc":"2.0","method":"test","data":"{}","id":1}}"#,
            content
        )
    })
}
...
mod tests {
    ...
    fn test_framer_empty_lines() {
        ...
        assert!(
            messages.is_empty(),
            "Empty lines should produce no messages"
        );
    }
    ...
    #[test]
    fn test_stdio_transport_rejects_configured_oversize() {
        let config = StdioConfig {
            max_message_size: 8,
            auto_flush: true,
            stderr_logging: false,
        ...
        let mut reader = Cursor::new("0123456789\n");
    ...
    #[test]
    fn test_stdio_transport_oversize_stops_before_consuming_entire_line() {
        let config = StdioConfig {
            max_message_size: 8,
            auto_flush: true,
            stderr_logging: false,
        ...
        let mut transport = StdioTransport::with_config(config);
        let mut reader = Cursor::new("0123456789abcdefghijklmnopqrstuvwxyz\n");
    ...
    #[test]
    fn test_stdio_write_message_rejects_configured_oversize() {
        let config = StdioConfig {
            max_message_size: 8,
            auto_flush: true,
            stderr_logging: false,
        ...
        let result = transport.write_message(&mut output, "0123456789");
    ...
    #[test]
    fn test_stdio_write_message_rejects_after_shutdown_without_output() {
        let transport = StdioTransport::new();
        transport.shutdown();
        let mut output = Vec::new();

        let result = transport.write_message(&mut output, r#"{"jsonrpc":"2.0","id":1}"#);
    ...
    #[test]
    fn test_stdio_log_line_redacts_secret_text() {
        let line = StdioTransport::format_log_line("DCP DEBUG", "apiKey=abc123");

        assert!(line.contains("[DCP DEBUG] [REDACTED]"));
        assert!(!line.contains("abc123"));
        assert!(!line.contains("apiKey="));
    }

    #[test]
    fn test_message_framer_clears_buffer_after_oversize() {
        let mut framer = MessageFramer::new().with_max_size(4);

        let result = framer.feed(b"abcde");
