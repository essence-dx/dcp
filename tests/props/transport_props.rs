...
use dcp::compat::sse::SseMessageHandler;
use dcp::transport::{
    framing::{
        FrameCodec, FrameError, FrameHeader, FRAME_HEADER_SIZE, MAX_MESSAGE_SIZE, PROTOCOL_VERSION,
    },
    tcp::{Connection, EchoHandler, ProtocolMode, TcpConfig, TcpServer, TlsConfig, DCP_MAGIC},
};
...
mod tests {
    ...
    #[test]
    fn sse_post_rejects_cancelled_notification_extra_params_before_accept() {
        let handler = SseMessageHandler::new();
        let request = r#"{"jsonrpc":"2.0","method":"notifications/cancelled","params":{"requestId":7,"reason":"ok","api_key":"plain-secret"}}"#;

        let error = handler
            .handle_message(request)
            .expect_err("extra cancelled params should be rejected");

        assert_eq!(error, "Invalid JSON-RPC notification params");
    ...
    #[tokio::test]
    async fn tcp_tls_configuration_fails_closed_until_tls_acceptor_is_wired() {
        let config = TcpConfig {
            bind_addr: "127.0.0.1:0".parse().unwrap(),
            tls: Some(TlsConfig::new("missing-cert.pem", "missing-key.pem")),
            ..Default::default()
        };
        let server = TcpServer::new(config);
        ...
        let result = tokio::time::timeout(
            ...
            server.run(EchoHandler),
        )
        .await;

        let error = result
            .expect("TLS-configured server should fail closed without waiting for clients")
            .expect_err("TLS-configured server must not silently serve plaintext");
