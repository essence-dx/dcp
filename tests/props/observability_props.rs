...
use dcp::observability::{
    create_span, LogConfig, LogEntry, LogFormat, LogLevel, LogValue, MetricsConfig,
    PrometheusMetrics, RequestIdGenerator, RequestMetrics, RequestSpan, Span, SpanKind, SpanStatus,
    SpanValue, StructuredLogger, Tracer, TracingConfig,
};
use std::sync::Arc;
...
mod tests {
    ...
    #[test]
    fn test_log_entry_redacts_sensitive_fields() {
        let entry = LogEntry::new(LogLevel::Info, "request accepted")
    ...
    #[test]
    fn test_log_json_escapes_field_keys() {
        let entry =
            LogEntry::new(LogLevel::Info, "request accepted").with_field("bad\"key", "value");

        let json = entry.to_json("test-service");
        let parsed: serde_json::Value = serde_json::from_str(&json).unwrap();

        assert_eq!(parsed["bad\"key"], "value");
    }

    #[test]
    fn test_log_entry_redacts_secret_bearing_field_keys() {
        let entry = LogEntry::new(LogLevel::Info, "request accepted")
        ...
        assert!(!json.contains("sk-live-key-in-field-name"));
        assert!(!json.contains("access_token=plain-secret"));
        assert!(!json.contains("plain-secret"));
        assert!(!text.contains("sk-live-key-in-field-name"));
        assert!(!text.contains("access_token=plain-secret"));
        assert!(!text.contains("plain-secret"));
    }

    #[test]
    fn test_log_entry_rendering_and_emit_sanitize_public_mutation() {
        let mut entry = LogEntry::new(LogLevel::Info, "request accepted");
        entry.message = "api_key=message-secret".to_string();
        entry.request_id = Some("access_token=request-secret".to_string());
        entry.trace_id = Some("Bearer trace-secret".to_string());
        entry.file = Some("private_key=file-secret".to_string());
        entry.fields.insert(
            "safe".to_string(),
            LogValue::String("authorization=field-secret".to_string()),
        );
        entry.fields.insert(
            "api_key=key-secret".to_string(),
            LogValue::String("visible".to_string()),
        );
        ...
        assert_eq!(parsed["file"], "[REDACTED]");
        ...
        assert_eq!(entries.len(), 1);
        assert!(!entries[0].message.contains("message-secret"));
        assert!(!format!("{:?}", entries[0].request_id).contains("request-secret"));
        assert!(!format!("{:?}", entries[0].trace_id).contains("trace-secret"));
        assert!(!format!("{:?}", entries[0].file).contains("file-secret"));
        assert!(!format!("{:?}", entries[0].fields).contains("field-secret"));
        assert!(!format!("{:?}", entries[0].fields).contains("key-secret"));
    }

    #[test]
    fn test_span_attributes_redact_sensitive_values() {
        let mut span = create_span("tools/call");
        span.set_attribute("authorization", "Bearer span-secret");

        let value = span.attributes.get("authorization").unwrap();

        assert!(matches!(value, SpanValue::String(s) if s == "[REDACTED]"));
    }

    #[test]
    fn test_span_event_attributes_redact_sensitive_values() {
        let mut span = create_span("tools/call");
        let mut attributes = HashMap::new();
        attributes.insert(
            "authorization".to_string(),
            SpanValue::String("Bearer event-secret".to_string()),
        );

        span.add_event_with_attributes("failed Bearer event-secret", attributes);

        let event = span.events.last().unwrap();
        let value = event.attributes.get("authorization").unwrap();

        assert!(!event.name.contains("event-secret"));
        assert!(matches!(value, SpanValue::String(s) if s == "[REDACTED]"));
    }

    #[test]
    fn test_span_attributes_redact_secret_bearing_keys() {
        let mut span = create_span("tools/call");
        span.set_attribute("sk-live-key-in-field-name", "safe-value");

        assert!(span.attributes.contains_key("[REDACTED]"));
        assert!(!span.attributes.contains_key("sk-live-key-in-field-name"));
    }

    #[test]
    fn test_span_event_attributes_redact_secret_bearing_keys() {
        let mut span = create_span("tools/call");
        let mut attributes = HashMap::new();
        attributes.insert(
            "access_token=plain-secret".to_string(),
            SpanValue::String("safe-value".to_string()),
        );

        span.add_event_with_attributes("event", attributes);

        let event = span.events.last().unwrap();
        assert!(event.attributes.contains_key("[REDACTED]"));
        assert!(!event.attributes.contains_key("access_token=plain-secret"));
    }

    #[test]
    fn test_span_names_redact_sensitive_text() {
        let span = create_span("tools/call?api_key=span-secret");
        let request = RequestSpan::new(
            "tools/call?authorization=Bearer span-secret",
            "req-001".to_string(),
        )
        .finish();

        assert_eq!(span.name, "[REDACTED]");
        assert_eq!(request.name, "[REDACTED]");
        assert!(!span.name.contains("span-secret"));
        assert!(!request.name.contains("span-secret"));
    }

    #[test]
    fn test_prometheus_labels_redact_and_escape_sensitive_methods() {
        let metrics = PrometheusMetrics::with_defaults();

        metrics.record_request(
            "tools/call?api_key=metrics-secret\"}\ndcp_injected 1",
            Duration::from_millis(5),
        );

        let output = metrics.format_prometheus();

        assert!(!output.contains("metrics-secret"));
        assert!(!output.contains("dcp_injected"));
        assert!(output.contains("dcp_requests_total{method=\"[REDACTED]\"}"));
    }
