...
use dcp::{CapabilityManifest, SecurityError};
...
proptest! {
    ...
    /// Feature: dcp-security, Capability escalation prevention.
    /// Negotiated capabilities SHALL deny a tool unless both peers offered it.
    #[test]
    fn prop_negotiated_manifest_denies_tool_escalation(
        client_tool in 0u16..8192,
        server_tool in 0u16..8192,
        requested_tool in 0u16..8192,
    ) {
        let mut client = CapabilityManifest::new(1);
        let mut server = CapabilityManifest::new(1);
        client.set_tool(client_tool);
        server.set_tool(server_tool);

        let negotiated = CapabilityManifest::negotiate(&client, &server);
        let result = negotiated.require_tool(requested_tool);

        if requested_tool == client_tool && requested_tool == server_tool {
            prop_assert!(result.is_ok());
        } else {
            prop_assert_eq!(result, Err(SecurityError::InsufficientCapabilities));
        }
    }
