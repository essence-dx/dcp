...
mod tests {
    ...
    use dcp::compat::json_rpc::{
        JsonRpcError as DcpJsonRpcError, JsonRpcParseError, JsonRpcParser,
        JsonRpcResponse as DcpJsonRpcResponse, RequestId as DcpRequestId,
        DEFAULT_MAX_JSONRPC_REQUEST_SIZE,
    };

    const MAX_TEST_REQUEST_ID_STRING_BYTES: usize = 256;
    ...
    #[test]
    fn parser_rejects_duplicate_jsonrpc_request_fields() {
        let duplicate_method =
            r#"{"jsonrpc":"2.0","method":"tools/list","method":"tools/call","id":1}"#;

        let result = dcp::compat::JsonRpcParser::parse_request(duplicate_method);

        assert!(matches!(result, Err(JsonRpcParseError::DuplicateField)));
    }

    #[test]
    fn parser_rejects_duplicate_jsonrpc_response_fields() {
        let duplicate_result =
            r#"{"jsonrpc":"2.0","result":{"ok":true},"result":{"ok":false},"id":1}"#;

        let result = dcp::compat::JsonRpcParser::parse_response(duplicate_result);

        assert!(matches!(result, Err(JsonRpcParseError::DuplicateField)));
    }

    #[test]
    fn parser_rejects_unknown_jsonrpc_request_envelope_fields() {
        let request = r#"{"jsonrpc":"2.0","method":"tools/list","api_key":"plain-secret","id":1}"#;

        let result = dcp::compat::JsonRpcParser::parse_request(request);

        assert!(matches!(result, Err(JsonRpcParseError::InvalidStructure)));
    }

    #[test]
    fn parser_rejects_unknown_jsonrpc_response_envelope_fields() {
        let response =
            r#"{"jsonrpc":"2.0","result":{"ok":true},"access_token":"plain-secret","id":1}"#;

        let result = dcp::compat::JsonRpcParser::parse_response(response);

        assert!(matches!(result, Err(JsonRpcParseError::InvalidStructure)));
    }

    #[test]
    fn parser_rejects_request_fields_on_jsonrpc_response() {
        let response_with_method =
            r#"{"jsonrpc":"2.0","result":{"ok":true},"method":"tools/list","id":1}"#;
        let response_with_params = r#"{"jsonrpc":"2.0","result":{"ok":true},"params":{},"id":1}"#;

        assert!(matches!(
            dcp::compat::JsonRpcParser::parse_response(response_with_method),
            Err(JsonRpcParseError::InvalidStructure)
        ));
        assert!(matches!(
            dcp::compat::JsonRpcParser::parse_response(response_with_params),
            Err(JsonRpcParseError::InvalidStructure)
        ));
    }

    #[test]
    fn parser_rejects_control_character_method_names() {
        for method in [
            "tools/list\nx",
            "tools/list\rx",
            "tools/list\tx",
            ...
            let result = JsonRpcParser::parse_request(&request);
    ...
    #[test]
    fn parser_format_response_rejects_invalid_response_shapes() {
        let invalid_responses = [
            DcpJsonRpcResponse {
                jsonrpc: "1.0".to_string(),
                result: Some(json!({"ok": true})),
                error: None,
                id: DcpRequestId::Number(1),
            },
            DcpJsonRpcResponse {
                jsonrpc: "2.0".to_string(),
                result: Some(json!({"ok": true})),
                error: Some(DcpJsonRpcError::internal_error()),
                id: DcpRequestId::Number(1),
            },
            DcpJsonRpcResponse {
                jsonrpc: "2.0".to_string(),
        ...
        for response in invalid_responses {
            assert!(matches!(
                ...
                Err(JsonRpcParseError::InvalidVersion
                    | JsonRpcParseError::InvalidStructure
                    | JsonRpcParseError::MissingId)
            ));
        }
    }

    #[test]
    fn parser_rejects_null_params_on_requests_and_notifications() {
        for request in [
            r#"{"jsonrpc":"2.0","method":"tools/list","params":null,"id":1}"#,
            r#"{"jsonrpc":"2.0","method":"notifications/initialized","params":null}"#,
        ] {
            let result = JsonRpcParser::parse_request(request);
    ...
    #[test]
    fn parser_rejects_jsonrpc_batch_requests_explicitly() {
        for batch in [
            "[]",
            r#"[{"jsonrpc":"2.0","method":"tools/list","id":1}]"#,
            r#"[{"jsonrpc":"2.0","method":"tools/list","id":1},{"jsonrpc":"2.0","method":"tools/call","params":{"name":"secret"},"id":2}]"#,
        ] {
            let result = JsonRpcParser::parse_request(batch);
    ...
    #[test]
    fn parser_default_rejects_payload_above_default_request_limit() {
        let payload = "x".repeat(DEFAULT_MAX_JSONRPC_REQUEST_SIZE + 1);

        let result = JsonRpcParser::parse_request(&payload);

        assert!(matches!(result, Err(JsonRpcParseError::RequestTooLarge)));
    }

    #[test]
    fn parser_default_rejects_payload_above_default_response_limit() {
        let payload = "x".repeat(DEFAULT_MAX_JSONRPC_REQUEST_SIZE + 1);

        let result = JsonRpcParser::parse_response(&payload);

        assert!(matches!(result, Err(JsonRpcParseError::RequestTooLarge)));
    }

    #[test]
    fn parser_rejects_oversized_jsonrpc_request_id_string() {
        let id = "a".repeat(MAX_TEST_REQUEST_ID_STRING_BYTES + 1);
        let request = json!({
            "jsonrpc": "2.0",
            "method": "ping",
            "id": id
    ...
    #[test]
    fn parser_rejects_oversized_jsonrpc_response_id_string() {
        let id = "a".repeat(MAX_TEST_REQUEST_ID_STRING_BYTES + 1);
        let response = json!({
            "jsonrpc": "2.0",
            "result": {"ok": true},
        ...
        let result = JsonRpcParser::parse_response(&response);
