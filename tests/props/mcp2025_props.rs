...
// negotiate to the requested version if supported. Strict inbound request
// paths reject unsupported versions instead of silently downgrading.
// ============================================================================
...
fn arb_unsupported_version() -> impl Strategy<Value = String> {
    prop_oneof![
        ...
        Just("".to_string()),
        "[a-z0-9-]{1,20}".prop_filter("not a valid version", |s| {
            ProtocolVersion::from_str(s).is_none()
        }),
    ]
}
...
proptest! {
    ...
    /// Feature: mcp-2025-compatibility, Property 1: Strict Protocol Version Negotiation
    /// Unsupported versions are rejected by strict inbound request paths.
    #[test]
    fn prop_version_negotiation_strict_rejects_unsupported(version in arb_unsupported_version()) {
        let negotiator = VersionNegotiator::new();
        let negotiated = negotiator.try_negotiate(&version);

        prop_assert!(negotiated.is_none(),
            "Unsupported version {} should be rejected by strict negotiation", version);
    }

    /// Feature: mcp-2025-compatibility, Property 1: Protocol Version Negotiation (fallback)
    /// For any unsupported version string, negotiation falls back to least capable.
    #[test]
    fn prop_version_negotiation_fallback(version in arb_unsupported_version()) {
        let negotiator = VersionNegotiator::new();
        let negotiated = negotiator.negotiate(&version);

        // Should fall back without enabling newer optional capabilities.
        prop_assert_eq!(negotiated, ProtocolVersion::V2024_11_05,
            "Unsupported version {} should fall back to least capable", version);
    }
...
#[cfg(test)]
mod roots_security_tests {
    use super::*;

    #[tokio::test]
    async fn roots_empty_denies_file_uri_by_default() {
        let registry = RootsRegistry::new();

        assert!(!registry.allows_uri("file:///secret.txt").await);
        assert!(registry.allows_uri("https://example.com/resource").await);
    }
}
...
mod subscription_security_tests {
    ...
    #[tokio::test]
    async fn subscription_tracker_deduplicates_repeated_subscriber_for_resource() {
        let tracker = SubscriptionTracker::new();

        assert!(tracker.subscribe("file:///repeat.txt", "client-1").await);
        assert!(tracker.subscribe("file:///repeat.txt", "client-1").await);

        assert_eq!(tracker.subscription_count("file:///repeat.txt").await, 1);
        assert_eq!(
            tracker.get_subscribers("file:///repeat.txt").await,
            vec!["client-1".to_string()]
        );
    }

    #[tokio::test]
    async fn subscription_tracker_rejects_new_subscribers_over_per_resource_limit() {
        let tracker = SubscriptionTracker::with_max_subscribers_per_resource(2);

        assert!(tracker.subscribe("file:///limited.txt", "client-1").await);
        assert!(tracker.subscribe("file:///limited.txt", "client-2").await);
        assert!(!tracker.subscribe("file:///limited.txt", "client-3").await);

        assert_eq!(tracker.subscription_count("file:///limited.txt").await, 2);
        assert!(
            !tracker
                .is_subscribed("file:///limited.txt", "client-3")
                .await
        );
    }
}
