...
#[test]
fn write_chunk_backpressure_is_atomic() {
    let stream = DcpStream::new(1, 16);
    let oversized_for_buffer = vec![7u8; 10];

    let result = stream.write_chunk(&oversized_for_buffer, false);

    assert_eq!(result, Err(DCPError::Backpressure));
    assert_eq!(stream.next_send_seq(), 0);
    assert_eq!(stream.buffer().len(), 0);

    stream.write_chunk(b"ok", true).unwrap();
    assert_eq!(stream.next_send_seq(), 1);

    let (_chunk, payload) = stream.read_chunk().unwrap().unwrap();
    assert_eq!(payload, b"ok");
}

#[test]
fn write_chunk_rejects_writes_after_last_chunk_without_mutation() {
    let stream = DcpStream::new(1, 1024);

    stream.write_chunk(b"final", true).unwrap();
    let result = stream.write_chunk(b"replay", false);

    assert_eq!(result, Err(DCPError::ValidationFailed));
    assert_eq!(stream.next_send_seq(), 1);

    let (chunk, payload) = stream.read_chunk().unwrap().unwrap();
