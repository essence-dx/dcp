...
export interface DcpClientConfig {
    ...
    dcpCapabilities?: DcpCapabilities;
}
...
export class DcpClient {
    ...
    private dcpCapabilities?: DcpCapabilities;
    ...
    constructor(options: DcpClientOptions | DcpClientConfig) {
        ...
        this.dcpCapabilities = this.normalizeDcpCapabilities(options.dcpCapabilities);
        this.setupMessageHandler();
    }

    private normalizeDcpCapabilities(capabilities?: DcpCapabilities): DcpCapabilities | undefined {
        if (!capabilities) {
            return undefined;
        }

        const normalized: DcpCapabilities = {};
        for (const key of ["tools", "resources", "prompts", "extensions"] as const) {
            const ids = capabilities[key];
            if (ids === undefined) {
                continue;
            }
            if (!Array.isArray(ids)) {
                throw new Error(`DCP capability '${key}' must be an array of ids`);
            }
            const copiedIds = ids.map((id) => {
                if (!Number.isInteger(id) || id < 0) {
                    throw new Error(`DCP capability '${key}' contains an invalid id`);
                }
                return id;
            });
            if (copiedIds.length > 0) {
                normalized[key] = copiedIds;
            }
        }
    ...
    static async connectTcp(
        ...
        return new DcpClient({ transport, timeout, protocolVersion, dcpCapabilities });
    }
    ...
    async initialize(): Promise<InitializeResult> {
        ...
        if (this.dcpCapabilities) {
            capabilities.dcp = {
                ...this.dcpCapabilities,
            };
        }
