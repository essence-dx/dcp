...
@pytest.mark.asyncio
async def test_initialize_sends_requested_dcp_capabilities(mock_transport):
    """Test initialize includes least-privilege DCP capability requests."""
    await mock_transport.connect()
    client = DcpClient(
        mock_transport,
        timeout=1.0,
    ...
    assert request["params"]["capabilities"]["dcp"] == {
        "tools": [7],
        "resources": [2],
        "prompts": [3],
        "extensions": [1],
